...
var ToText = function(rrule, gettext, language) {
    ...
    this.monthDaySorter = function(a,b) {return a-b};
    if (typeof this.origOptions.bymonthday !== 'undefined') {
        var bymonthday = [].concat(this.options.bymonthday);
        var bynmonthday = [].concat(this.options.bynmonthday);
        bymonthday.sort(this.monthDaySorter);
        bynmonthday.sort(this.monthDaySorter);
        bynmonthday.reverse();
        // 1, 2, 3, .., -5, -4, -3, ..
    ...
    if (typeof this.origOptions.byweekday !== 'undefined') {
        var byweekday = !(this.origOptions.byweekday instanceof Array)
                            ? [this.origOptions.byweekday]
                            : this.origOptions.byweekday;
        var tmpbysetpos = this.origOptions.bysetpos;
        var days = String(byweekday);
        this.byweekday = {
            allWeeks:byweekday.filter(function (weekday) {
                return !Boolean(weekday.n) && typeof tmpbysetpos === 'undefined';
            }),
...
ToText.prototype = {
    ...
    weekdaytext: function(wday, bysetpos) {
        var jsWeekday = isNaN(wday) ? wday.getJsWeekday() : wday == 6 ? 0 : wday + 1;
        return (wday.n ? this.nth(wday.n) + ' ' : '')
            + this.language.dayNames[jsWeekday];
    },
    ...
    list: function(arr, callback, finalDelim, delim, bysetpos) {
        ...
        bysetpos = typeof bysetpos !== 'undefined' && bysetpos !== null
                            ? (!(bysetpos instanceof Array)
                                ? [bysetpos]
                                : bysetpos)
                            : undefined;

        if(bysetpos && (bysetpos.length === 1 || bysetpos.length === arr.length)) {
            for(var i=0;i<arr.length;i++) {
                if(isNaN(arr[i]) && typeof arr[i].n === 'undefined') {
                    arr[i] = {
                        weekday:arr[i].weekday,
                        n: bysetpos.length === 1 ? bysetpos[0]: bysetpos[i],
