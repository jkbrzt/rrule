...
;(function (root, factory) {
  ...
  var dateutil = {
    ...
    untilStringToDate: function (until, timeZone) {
      ...
      return dateutil.createDate(Date.UTC(
        bits[1],
        bits[2] - 1,
        bits[3],
        bits[5] || 0,
    ...
    */
    ...
    _cacheGet: function (what, args) {
      ...
      return cached instanceof Array
        ...
        : (dateutil.isDate(cached) ? dateutil.clone(cached) : cached)
    },
    ...
    if (year !== this.lastyear) {
      ...
      if (!plb(rr.options.byweekno)) {
        ...
        if (no1wkst) {
          ...
          var lnumweeks
          if (!contains(rr.options.byweekno, -1)) {
            var lyearweekday = dateutil.getWeekday(dateutil.createDate(year - 1, 0, 1, timeZone));
  ...
  Iterinfo.prototype.wdayset = function (year, month, day) {
    ...
    var i = dateutil.toOrdinal(dateutil.createDate(year, month - 1, day, timeZone)) - this.yearordinal;
    var start = i
    for (var j = 0; j < 7; j++) {
      set[i] = i
      ++i
    ...
    init: function (method, args) {
      ...
      if (method === 'between') {
        this.maxDate = args.inc
          ? before : dateutil.createDate(before.getTime() - 1, timezone);
        this.minDate = args.inc
          ? after : dateutil.createDate(after.getTime() + 1, timezone);
      } else if (method === 'before') {
        this.maxDate = args.inc ? args.dt : dateutil.createDate(args.dt.getTime() - 1, timezone);
      } else if (method === 'after') {
        this.minDate = args.inc ? args.dt : dateutil.createDate(args.dt.getTime() + 1, timezone);
      }
    },
  ...
  RRuleSet.prototype = {
    ...
    */
    ...
    _iter: function (iterResult) {
      ...
      var timezone = this._rrule.options.timezone;

      function evalExdate (after, before) {
        _exrule.forEach(function (rrule) {
          rrule.between(after, before, true).forEach(function (date) {
            date = dateutil.createDate(date.valueOf(), timezone);
      ...
      if (iterResult.method === 'between') {
        ...
        iterResult.accept = function (date) {
          date = moment.tz(date, _timezone).toDate();
          var dt = Number(date)
          if (!_exdateHash[dt]) {
            _exdateHash[dt] = true
            return _accept.call(this, date)
      ...
      for (var i = 0; i < this._rdate.length; i++) {
        if (!iterResult.accept(dateutil.createDate(this._rdate[i].valueOf(), timezone))) break
      }
    ...
    */
    clone: function () {
      ...
      for (i = 0; i < this._rdate.length; i++) {
        rrs.date(dateutil.createDate(this._rdate[i], timezone));
      }
      for (i = 0; i < this._exrule.length; i++) {
        rrs.exrule(this._exrule[i].clone())
      }
      for (i = 0; i < this._exdate.length; i++) {
        rrs.exdate(dateutil.createDate(this._exdate[i], timezone));
      }
      return rrs
    }
  }
      ...
      if (!options.forceset && lines.length === 1 && (s.indexOf(':') === -1 ||
        ...
        for (i = 0; i < lines.length; i++) {
          ...
          if (name === 'RRULE') {
            ...
            dtstart = dateutil.untilStringToDate(value) //TODO: pass TZ into the rest of untilStringToDate refs.
          } else {
