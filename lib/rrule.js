...
/**
 * GenItem
 *  Used in RRuleSet's remove algorithm in _iter method
 */

var GenItem = function (genlist, gen, method, args) {
    if (gen instanceof RRule) {
      if (method != 'before' && method != 'after') {
        this.dts = gen[method].apply(gen, args);
      } else {
        this.dts = gen.all()
      }
    } else {
...
GenItem.prototype.next = function () {
    this.dt = this.dts[this.index++];
    if (!this.dt) {
        this.genlist.splice(this.genlist.indexOf(this), 1);
    }
};
...
RRuleSet.prototype = {
    ...
    * @param {Date}
    */
    exdate: function (exdate) {
        this._exdate.push(exdate);
    },
    valueOf: function () {
        var result = []
    ...
    */
    ...
    _iter: function (iterResult) {
        var rlist = [];
        this._rdate = this._rdate.sort(function (current, next) {
            return current > next;
        })
        this._genitem(rlist, this._rdate);
        // Pass iterResult to rrule to limit generated date count
        this._rrule.forEach(function (rrule) {
            var args = [], method;
            method = iterResult.method
            if (method == 'between') {
              args.push(new Date(iterResult.args.after));
              args.push(new Date(iterResult.args.before));
              args.push(iterResult.args.inc);
            } else if (['before', 'after'].indexOf(method) > -1) {
              args.push(new Date(iterResult.args.dt));
              args.push(iterResult.args.inc);
            }
            this._genitem(rlist, rrule, method, args);
        }.bind(this));
        rlist = rlist.sort(function (current, next) {
            return current.dt > next.dt;
        });
        var exlist = []
        this._exdate = this._exdate.sort(function (current, next) {
            return current > next;
        });
        this._genitem(exlist, this._exdate)
        this._exrule.forEach(function (exrule) {
            var args = [], method;
            method = iterResult.method
            if (method == 'between') {
            ...
            this._genitem(exlist, exrule, method, args);
        }.bind(this));
        exlist = exlist.sort(function (current, next) {
            return current.dt > next.dt;
        });
        var lastdt = null;
        var total = 0;
        ...
        while (rlist.length) {
            ritem = rlist[0];
            if (!lastdt || lastdt.getTime() != ritem.dt.getTime()) {
                while(exlist.length && exlist[0].dt.getTime() < ritem.dt.getTime()) {
                    exlist[0].next();
                    exlist = exlist.sort(function (current, next) {
                        return current.dt > next.dt;
                    })
                }
                if (!exlist.length || ritem.dt.getTime() != exlist[0].dt.getTime()) {
                    total += 1;
                    if (!iterResult.accept(ritem.dt)) {
                        return iterResult.getValue();
                    }
                }
                lastdt = ritem.dt;
    ...
    _genitem: function (list, gen, method, args) {
        new GenItem(list, gen, method, args);
    },
...
['all', 'between', 'before', 'after', 'count', '_cacheAdd', '_cacheGet'].forEach(function (method) {
...
RRuleStr._freq_map = {"YEARLY": RRule.YEARLY,
...
RRuleStr._weekday_map = {"MO": 0, "TU": 1, "WE": 2, "TH": 3,
                    "FR": 4, "SA": 5, "SU": 6};
...
RRuleStr.prototype = {
    ...
    _handle_int: function (rrkwargs, name, value, options) {
        rrkwargs[name.toLowerCase()] = parseInt(value);
    },

    _handle_int_list: function (rrkwargs, name, value, options) {
        rrkwargs[name.toLowerCase()] = value.split(',').map(function(x) { return parseInt(x)});
    },
    ...
    parse: function (s, options) {
        ...
        var invalid = [],
            keys = Object.keys(options),
            defaultKeys = Object.keys(RRuleStr.DEFAULT_OPTIONS);
...
if (typeof define === "function" && define.amd) {
    ...
    define("rrule", [], function () {
        ...
        // Always return RRule for Backward Compatibility
        return RRule;
    });
}
