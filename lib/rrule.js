...
var dateutil = {
    ...
    fromOrdinal: function(ordinal, timeZone) {
        ...
        var value = dateutil.ORDINAL_BASE.getTime()
            ...
            +  millisecsFromBase
            + dateutil.tzOffset(dateutil.createDate(millisecsFromBase));
    ...
    clone: function(date) {
        var dolly = dateutil.createDate(date.getTime());
    ...
    timeToUntilString: function(time) {
        var date = dateutil.createDate(time);
        var comp, comps = [
            date.getUTCFullYear(),
            date.getUTCMonth() + 1,
            date.getUTCDate(),
    ...
    createDate: function(args, timeZone) {
        TODO: ALLOW ARGUMENTS TO BE VARIADIC.
        args = args || [];
        if (args.length == null) {
            args = [args];
        }
...
IterResult.prototype = {
    ...
    init: function(method, args) {
        ...
        if (method == 'between') {
            this.maxDate = args.inc
                ? before
                : dateutil.createDate(before.getTime() - 1);
            this.minDate = args.inc
                ? after
                : dateutil.createDate(after.getTime() + 1);
        } else if (method == 'before') {
            this.maxDate = args.inc ? args.dt : dateutil.createDate(args.dt.getTime() - 1);
        } else if (method == 'after') {
            this.minDate = args.inc ? args.dt : dateutil.createDate(args.dt.getTime() + 1);
        }
    },
     ...
     */
    add: function(date) {
        this._result.push(dateutil.createDate(date.valueOf()));
        return true;
    },
