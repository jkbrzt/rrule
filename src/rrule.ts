    ...
    for (let i = 0; i < attrs.length; i++) {
      ...
      switch (key) {
        ...
        case "BYDAY": // => byweekday
          let n: number, wday: Weekday | number, day: string;
    ...
    */
    ...
    while (true) {
      ...
      if (plb(bysetpos) && plb(timeset)) {
        let daypos: number, timepos: number;
        const poslist: Date[] = [];
      ...
      if (plb(ranges)) {
        ...
        for (let j = 0; j < ranges.length; j++) {
          ...
          for (let k = 0; k < rr.options.bynweekday.length; k++) {
            ...
            if (n < 0) {
              i = last + (n + 1) * 7;
              i -= pymod((this.wdaymask[i] as number) - wday, 7);
            } else {
              i = first + (n - 1) * 7;
              i += pymod(7 - (this.wdaymask[i] as number) + wday, 7);
            }
