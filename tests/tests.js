/* global $, test, equal, RRule, RRuleSet, rrulestr
  testRecurring, datetimeUTC, datetime, parse, assertStrType*/
...
test('testAfterBefore', function () {
  'YEARLY,MONTHLY,DAILY,HOURLY,MINUTELY,SECONDLY'.split(',').forEach(function (freq_str) {
    ...
    equal(date.getTime(), rr.options.dtstart.getTime(),
    ...
    equal(res, rr.options.dtstart.getTime(),
